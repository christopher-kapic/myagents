...
agentCommand
  .command("start-conversation <slug> <message>")
  .description("Start a new conversation as an agent (agent proactively messages the user)")
  .option("--title <title>", "Conversation title (defaults to first 50 chars of message)")
  .option("--server <url>", "Server URL (overrides env/config)")
  .option("--api-key <key>", "API key (overrides env/config)")
  ...
  .action(
    async (
      slug: string,
      message: string,
      opts: { title?: string; server?: string; apiKey?: string; timeout?: string; json?: boolean },
    ) => {
      const apiKey = opts.apiKey ?? resolveApiKey();
      if (!apiKey) {
        console.error(
      ...
      try {
        const result = await startConversationViaWebSocket(
          serverUrl,
          apiKey,
          slug,
          message,
...
/**
 * Start a new conversation as an agent via a temporary WebSocket connection.
 * The agent proactively creates a conversation and sends the first message.
 */
function startConversationViaWebSocket(
  serverUrl: string,
  apiKey: string,
  agentSlug: string,
  message: string,
  ...
  return new Promise((resolve, reject) => {
    ...
    // Connect as client (same as agent send) — the server resolves the agent by slug + userId
