...
self.addEventListener("push", (event) => {
    let data = { title: "Notification", body: "", data: {} };
    if (event.data) {
        try {
...
self.addEventListener("pushsubscriptionchange", ((event) => {
    ...
    pushEvent.waitUntil((async () => {
        ...
        if (!oldSubscription?.options)
            return;
        const newSubscription = await self.registration.pushManager.subscribe(oldSubscription.options);
