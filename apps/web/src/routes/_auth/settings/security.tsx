  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          {has2FA ? <Disable2FASection /> : <Enable2FASection />}
        </CardContent>
      </Card>
    </div>
  );
...
function Enable2FASection() {
  ...
  if (step === "idle") {
    return (
      <Button onClick={() => setStep("password")}>Enable Two-Factor Authentication</Button>
    );
  }
  ...
  if (step === "setup") {
    return (
      <div className="space-y-4 max-w-sm">
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            Add this key to your authenticator app (Authy, Google Authenticator, etc.):
          </p>
        ...
        <Button onClick={() => setStep("verify")}>I've saved my backup codes</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 max-w-sm">
      ...
      <Button onClick={handleVerify} disabled={verifyCode.length !== 6 || isLoading}>
